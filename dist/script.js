...
window.addEventListener('DOMContentLoaded', function () {
  var slider = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_0__["MainSlider"]({
    btns: '.next',
    container: '.page'
  }); // экземпляр класса
  ...
  showUpSlider.init();
  var modulesSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_1__["MiniSlider"]({
  ...
  modulesSlider.init();
  var feedSlider = new _modules_slider_slider_mini__WEBPACK_IMPORTED_MODULE_1__["MiniSlider"]({
  ...
  feedSlider.init();
  new _modules_difference__WEBPACK_IMPORTED_MODULE_2__["Difference"]('.officerold', '.officernew', '.officer__card-item').init();
  new _modules_forms__WEBPACK_IMPORTED_MODULE_4__["Form"]('.form').init();
  var player = new _modules_playVideo__WEBPACK_IMPORTED_MODULE_3__["VideoPlayer"]('.showup .play', '.overlay'); //player.init();
});
    ...
    value: function initMask() {
      var setCursorPosition = function setCursorPosition(pos, elem) {
        ...
        if (elem.setSelectionRange) {
          ...
          var range = elem.createTextRange(); // диапазон
      ...
      function createMask(event) {
        var matrix = '+1 (___) ___-____',
            ...
            // получаем все не цифры // статичная
        ...
        this.value = matrix.replace(/./g, function (a) {
          // . -> каждый элемент сущ-ет в строке
          return /[_\d]/.test(a) && i < val.length ? val.charAt(i++) : i >= val.length ? '' : a; // charAt возвращает символ, стоящий на указанной позиции в строке
          // \d -> все цифры, [] -> диапазон
        });

        if (event.type === 'blur') {
          // blur - событие когда мы кликнули вне инпута
      ...
      inputs.forEach(function (input) {
        ...
        input.addEventListener('blur', createMask); // focus вызывается в момент фокусировки
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function bindCloseBtn() {
      ...
      this.close.addEventListener('click', function () {
        _this2.overlay.style.display = 'none'; // при нажатии на крестик -> блок не закрывается

        _this2.player.stopVideo(); // при нажатии на крестик -> видео остнавливается 
    ...
    value: function createPlayer(url) {
      this.player = new YT.Player('frame', {
        //'frame' -> это ID на странице index.html
        height: '100%',
        width: '100%',
        videoId: "".concat(url)
      });
    ...
    value: function init() {
      ...
      var firstScriptTag = document.getElementsByTagName('script')[0]; // находим 1-й скрипт на странице

      firstScriptTag.parentNode.insertBefore(tag, firstScriptTag);
      /* Обращаемся к главному родителю (это) 
      будет body) и прямо перед первым нашим скриптом (firstScriptTag) помещаем переменную (tag)*/

      this.bindTriggers();
      this.bindCloseBtn(); //this.bindCloseModal();
    }
  }]);
...
function (_Slider) {
  ...
  return MainSlider;
}(_slider__WEBPACK_IMPORTED_MODULE_9__["Slider"]);
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          if (_this2.slides[i].tagName !== 'BUTTON') {
            var active = _this2.slides[i]; // последний слайд в нашем списке 

            _this2.container.insertBefore(active, _this2.slides[0]); // поместить перед this.slides[0]
